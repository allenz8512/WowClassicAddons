...
function Anchor:AddUnit(unit)
    if (self.units[unit.guid]) then 
        -- print("Anchor:AddUnit(unit) 已存在：" .. unit.guid) 
        return end -- trying to add existing unit to anchor (bad)

	-- unit now belongs here, update its parent
	unit:SetParent(self)
	unit.parentAnchor = self
    -- print("Anchor:AddUnit  unit.parentAnchor:" .. "guid:" .. unit.guid)
    -- print("unit")
    -- print(unit)
