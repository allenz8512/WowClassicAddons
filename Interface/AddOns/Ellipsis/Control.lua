...
function Ellipsis:ApplyOptionsUnitGroups()
    ...
    if (trackPlayer) then
        self:UNIT_AURA("player", true)
    end
    if (trackPet) then
        self:UNIT_AURA("pet", true)
    end
end
...
do ------------------------
    ...
    function Ellipsis:COMBAT_LOG_EVENT_UNFILTERED(
        ...
        -- else
        --     print(subEvent)
        --     print(timestamp .. ";当前事件：" .. (subEvent) .. ";sourceGUID:" .. (sourceGUID or "nil") .. ";sourceName:" .. (sourceName or "nil") .. ";destGUID:" .. (destGUID or "nil") .. ";destName:" .. (destName or "nil"))
        -- end
        if (deathEvents[subEvent]) then
            ...
            if (not destGUID or bit_band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == 0) then
                return
            end -- abort if no target, or we're not the source

            if (subEvent == "SPELL_CAST_SUCCESS") then -- used for tracking notarget spell casts
                if
                    (noTargetFake[arg1] and
                        ((filterByBlacklist and not blacklist[spellID]) or
                            (not filterByBlacklist and whitelist[spellID])))
                 then
                    local duration = noTargetFake[arg1]

                    if (duration <= durationMin or duration >= durationMax) then
                        return
                    end -- abort if duration is restricted
                    ...
                    local unit =
                        activeUnits["notarget"] or
                        Unit:New(GetTime(), "notarget", false, "notarget", L.UnitName_NoTarget, false, 0)
...
function Ellipsis:PLAYER_TOTEM_UPDATE(slot)
    ...
    if (spellID) then -- proper totem
        ...
        if (not summonSpellID) then
            return
        end -- not even enough info to fake it
    ...
    if
        (((filterByBlacklist and blacklist[spellID]) or (not filterByBlacklist and not whitelist[spellID])) or
            (duration < durationMin or duration >= durationMax))
     then
        return
    end
    ...
    if (startTime > 0) then -- totem in this slot
        if (duration == 0) then
            return
        end -- totem in this slot, but no duration (nothing to track)
        ...
        if (spellID == 104317) then -- this summon is Wild Imps (special case)
            ...
            totemData[slot] =
                unit:AddAura(
                Aura:New(currentTime, unit, totemID, totemName, totemTexture, duration, startTime + duration, 0)
            )
        end
...
function Ellipsis:PLAYER_TARGET_CHANGED()
    ...
    if (targetGUID) then -- we had a previous target
        ...
        if (unit) then -- we have auras on the previous target
            ...
            if (anchor ~= unit.parentAnchor) then -- unit needs to be moved to a new anchor
                -- print("新anchor新增guid1")
                -- print("anchor ~= unit.parentAnchor targetGUID1:" .. targetGUID)

                -- print("unit")
                -- print(unit)
                -- print("unit.parentAnchor1:" )
                -- print(unit.parentAnchor)
                -- print("unit.parentAnchor:RemoveUnit1" .. unit.parentAnchor.RemoveUnit)
                unit.parentAnchor:RemoveUnit(targetGUID)
                -- print("旧anchor移除guid1")
                anchor:AddUnit(unit)
            else
                -- print(unit)
                -- print("anchor更新guid1")
    ...
    if (UnitExists("target")) then -- we have a new target
        ...
        if (unit) then -- we have auras on this unit
            ...
            if (anchor ~= unit.parentAnchor) then -- unit needs to be moved to a new anchor
                -- print("新anchor新增guid2")
                -- print("anchor ~= unit.parentAnchor targetGUID2:" .. targetGUID)
                -- print("unit")
                -- print(unit)
                -- print("unit.parentAnchor2:")
                -- print(unit.parentAnchor)
                -- print("unit.parentAnchor:RemoveUnit2" .. unit.parentAnchor.RemoveUnit)
                unit.parentAnchor:RemoveUnit(targetGUID)
                -- print("旧anchor移除guid2")
                anchor:AddUnit(unit)
            else
                -- print(unit)
                -- print("anchor更新guid2")
        ...
        self:UNIT_AURA("target", true) -- scan new target
    else
        targetGUID = false
    end
end
...
function Ellipsis:PLAYER_FOCUS_CHANGED()
    ...
    if (UnitExists("focus")) then -- we have a new focus
        ...
        if (focusGUID == targetGUID) then
            return
        end -- we don't update the data if the focus is the target (it has precedence)
        ...
        self:UNIT_AURA("focus", true) -- scan new focus
    else
        focusGUID = false
    end
end
...
-- UNIT_AURA
-- ------------------------
local unit_Expiretime = {}

function Ellipsis:UNIT_AURA(unitTag, fireByself)
    -- print("unitTag:" .. unitTag)
    ...
    local spellName, icon, stackCount, debuffType, duration, expireTime, unitCaster, _, _, spellID =
        UnitAura(unitTag, 1, filter)
    -- print(spellName, icon,stackCount, debuffType, duration, expireTime, unitCaster,   spellID)
    local guid = UnitGUID(unitTag)
    local unit = activeUnits[guid]

    if (not spellName and not unit) then
        return
    end -- none of our auras here, and not already tracking this unit, bailout

    local currentTime = GetTime()

    -- if not self:GetAurasDuration(spellName) then
    --     local spellDecr = GetSpellDescription(spellID)
    --     if spellDecr then
    --         local specs = {"^.-(%d+)秒内.*$","^.-持续(%d+)秒.*$"}
    --         local curDuration,nums = 0,0
    --         local curnums = 0
    --         for i,v in ipairs(specs) do
    --             curDuration,curnums = gsub(spellDecr, v, "%1")
    --             print(curDuration,curnums,spellDecr,nums)
    --             spellDecr = curnums >0 and curDuration or spellDecr
    --             nums = nums + curnums
    --         end
    --         curDuration = nums >0 and tonumber(curDuration) or nil
    --         if curDuration then
    --             self:SetAurasDuration(spellName, curDuration)
    --             print(format("新增%s持续时间为%s秒！", spellName, curDuration))
    --         else
    --             self:SetAurasDuration(spellName, 0)
    --         end
    --     end
    -- end

    -- duration = duration == 0 and self:GetAurasDuration(spellName) or duration
    unit_Expiretime[guid] = unit_Expiretime[guid] or {}
    --初始化unit数据表
    -- expireTime =
    --     expireTime == 0 and fireByself ~= true and (duration + GetTime()) or unit_Expiretime[guid][spellName] or
    --     expireTime
    -- if spellName then
    --     unit_Expiretime[guid][spellName] = fireByself ~= true and expireTime or unit_Expiretime[guid][spellName]
    -- --初始化unit的spellName数据表
    -- end

    -- check for quick bailout conditions
    if (unit and currentTime == unit.updated) then
        return
    end -- unit exists, but was updated already this frame
    if (not trackPlayer and guid == playerGUID) then
        return
    end -- unit is player, but not tracking player (done here because is a common event source)
    if (not trackPet and guid == petGUID) then
        return
    end -- unit is pet, but not tracking pet (done here because is a common event source)
    if (blacklistByGUID[guid]) then
        return
    end -- unit is blacklisted from display (temporary minion with no death 'event')
    ...
    while (spellName) do -- only scanning our auras on target
        if (unitCaster == "player" or unitCaster == "pet") then -- make sure player (or their pet) cast this aura
            ...
            --初始化unit的spellName数据表
            if
                (((filterByBlacklist and not blacklist[spellID]) or (not filterByBlacklist and whitelist[spellID])) and
                    ((duration > 0 and (duration > durationMin and duration < durationMax)) or
                        (duration == 0 and not blockPassive)))
             then
                ...
                if (noTargetRedirect[spellID]) then -- spell needs to be redirected to notarget unit
                    local noTarget =
                        activeUnits["notarget"] or
                        Unit:New(currentTime, "notarget", false, "notarget", L.UnitName_NoTarget, false, 0)
                    ...
                    if (not unit) then -- got this far, we'll (soon) have an aura to add, make a unit to place it on
                        local group =
                            (guid == playerGUID) and "player" or (guid == petGUID) and "pet" or
                            (filter == "HARMFUL|PLAYER") and "harmful" or
                            "helpful"
        ...
        duration = duration == 0 and self:GetAurasDuration(spellName) or duration
        -- expireTime = expireTime == 0 and (duration + GetTime()) or expireTime
        expireTime =
            expireTime == 0 and fireByself ~= true and (duration + GetTime()) or unit_Expiretime[guid][spellName] or
            expireTime
    end
    ...
    if (not unit) then
        return
    end -- no auras (and thus no units) passed the filters, we're done here
